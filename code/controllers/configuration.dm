...
/datum/configuration
	...
	var/drukhari_scaling_coeff = 6		//how much does the amount of players get divided by to determine drukhari, this may need tweaking - Wel
	...
	for(var/t in Lines)
		...
		if(type == "config")
			...
			switch(name)
				...
				if("drukhari_scaling_coeff")
					config.drukhari_scaling_coeff = text2num(value)
				if("probability")
